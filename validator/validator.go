...
var (
	EmailRX = regexp.MustCompile(constants.EmailRX)
)
...
func (v *Validator) IsValidDateTimeRange(dateTimeStartStr, dateTimeEndStr string) bool {
	...
	dateTimeStart, err := time.Parse(constants.TimeFormat, dateTimeStartStr)
	if err != nil {
		return false
	}

	dateTimeEnd, err := time.Parse(constants.TimeFormat, dateTimeEndStr)
	if err != nil {
		return false
	}
